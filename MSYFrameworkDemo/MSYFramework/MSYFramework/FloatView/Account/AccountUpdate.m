...
-(void)layoutAccountUpdateWithSuperView:(UIView *)superView{
...
-(void)bindPhone{
    ...
    NSString *body = [NSString stringWithFormat:@"mobile=%@&userid=%@&mobile_code=%@&usertoken=%@",phoneTxt.text,UserID,mobileCode,TOKEN];
    NSLog(@"%@",body);
    NSData *data = [body dataUsingEncoding:NSUTF8StringEncoding];
    ...
    [GZNetwork requesetWithData:url bodyData:data completeBlock:^(NSDictionary *dict, NSURLResponse *response, NSError *error) {
        ...
        if (dict != NULL) {
            ...
            if (code == 200) {
                ...
                BindPhoneSuccessView *view = [[BindPhoneSuccessView alloc] init];
                [self.superview addSubview:view];
                [view layoutAccountUpdateWithSuperView:self.superview AndAccount:phoneTxt.text AndOldAccount:Username];
                ...
                [userDic setObject:@"1" forKey:@"isphone"];
                [userDic setObject:mobileCode forKey:@"password"];
